...
module.exports = function cloudCommand(options) {
  const path  = require("path")
  const fs    = require("fs")
  const http  = require("https")

  // ─── Config ──────────────────────────────────────────────────────────────────
  const BRAIN_PATH    = require('os').homedir() + '/.gemini/antigravity/brain'
  const USER_EMAIL    = options.email
  const INGEST_KEY    = options.key
  const INGEST_URL    = options.url
  const POLL_INTERVAL = 15000 // 15 seconds

  // Validate required options
  if (!USER_EMAIL) {
    console.error("❌ Missing required Google Email (--email)")
    process.exit(1)
  }
  if (!INGEST_KEY) {
