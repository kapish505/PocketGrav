/**
 * app/login/page.jsx — Login Page
 * ────────────────────────────────
 * Beautiful dark Google sign-in page.
 * Only reachable when not authenticated.
 */
"use client"

import { signIn } from "next-auth/react"

export default function LoginPage() {
  return (
    <div className="page-wrapper">
      <div className="login-page">
        <div className="card login-card">

          {/* Logo */}
          <div className="login-logo">
            <svg width="52" height="52" viewBox="0 0 24 24" fill="none">
              <path d="M12 2L2 7l10 5 10-5-10-5z" stroke="#7c3aed" strokeWidth="2" strokeLinejoin="round"/>
              <path d="M2 17l10 5 10-5" stroke="#7c3aed" strokeWidth="2" strokeLinejoin="round"/>
              <path d="M2 12l10 5 10-5" stroke="#a78bfa" strokeWidth="1.5" strokeLinejoin="round"/>
              <circle cx="12" cy="12" r="2" fill="#a78bfa" opacity="0.6"/>
            </svg>
          </div>

          {/* Title */}
          <h1 className="login-title">PocketGrav</h1>
          <p className="login-subtitle">
            Your AntiGravity projects, live on your phone.<br />
            Sign in to see your active sessions.
          </p>

          {/* Feature list */}
          <div className="login-features">
            <div className="login-feature">
              <span className="login-feature-icon">⚡</span>
              <span>Live task progress from your laptop</span>
            </div>
            <div className="login-feature">
              <span className="login-feature-icon">🧠</span>
              <span>All AntiGravity agent conversations</span>
            </div>
            <div className="login-feature">
              <span className="login-feature-icon">🌐</span>
              <span>Works from anywhere — no IP needed</span>
            </div>
          </div>

          {/* Google sign-in button */}
          <button
            className="btn-google"
            onClick={() => signIn("google", { callbackUrl: "/dashboard" })}
          >
            {/* Official Google G logo */}
            <svg width="20" height="20" viewBox="0 0 24 24">
              <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/>
              <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/>
              <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/>
              <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/>
            </svg>
            Continue with Google
          </button>

          <p className="login-note">
            Use your AntiGravity Gmail account
          </p>
        </div>
      </div>
    </div>
  )
}
